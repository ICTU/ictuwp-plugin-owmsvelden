 ...
 * Version:       1.1.3
 * Version desc:  Removed duplicate, lowercase LICENSE file.
...
if ( ! class_exists( 'OWMSvelden' ) ) :
	...
	class OWMSvelden {
		...
		public function get_header_data( $postid ) {
			...
			} elseif ( $currentposttype === 'event' ) {
				$owms_type = 'evenement';
			} elseif ( $currentposttype === 'community' ) {
				$owms_type = 'community';
			}
			...
			$owms_identifier = wp_strip_all_tags( $owms_identifier );
			$owms_title      = wp_strip_all_tags( $owms_title );
			$owms_type       = wp_strip_all_tags( $owms_type );
			$owms_language   = wp_strip_all_tags( $owms_language );
